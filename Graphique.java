...
public class Graphique extends JFrame {
  ...
  private void jbInit() throws Exception  {
    ...
    exp.setText("( 24 + 9 ) / ( 16 * 5 ) - ( 3 + 11 ) * "+
                "( ( 4 + 7 ) - ( 9 + 13 )"+
               " / ( 14 * 8 ) )");
   v.arbre=new ArbreExpression(java.lang.String.class);
expr=new ExpressionInfixee(exp.getText());
    int i=0;
     Panneau.add(new JScrollPane(v),BorderLayout.CENTER);
    while(i<expr.Prefixee().length()){
     String c=this.Extraire(i,expr.Prefixee());
     v.ajoute(c);
      i+=c.length()+1;
    }
    jLabel8.setText(exp.getText());
    jLabel9.setText(expr.Prefixee());
  ...
  public void jMenuHelpAbout_actionPerformed(ActionEvent e) {
    JOptionPane.showMessageDialog(this," Ecole Superieure d'Infotronique d'Haiti (ESIH)"+
                                  " Projet Algorthmique"+
                                  "Prepare par:"+
                                  "James PIERRE-LOUIS;"+
                                  "TOUSSAINT Benjy;"+
                                  "BLANC Jean Davidson;"+
                                  "L3_Soir"
                                  ,"ArbreE Expression Arithmetique"
...
exp.setText(exp.getText().substring(0,exp.getText().length()-1));
  ...
  void jButton18_actionPerformed(ActionEvent e) throws PileVideException,
      PileSatureeException, TypeIncompatibleException,
